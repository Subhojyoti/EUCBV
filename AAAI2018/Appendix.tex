...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\leq \dfrac{182 K^4 T^{\frac{5}{4}}\log T}{T^3} \overset{(e)}{\leq} \dfrac{182 K^4}{T^{\frac{5}{4}}} \overset{(f)}{\leq} \dfrac{182 K^4}{T^{\frac{5}{4}}\sqrt{\epsilon_{m_i}}}
\end{align*}

where, $(a)$ comes from the total pulls allocated for all $i\in B_m$ till the $m$-th round, in $(b)$ the arm count $|B_m|$ can be bounded by using $(\ref{eq:arm:elim:c1})$ and also we substitute the value of $n_{m}$, $(c)$ happens by substituting the value of $\psi$ and considering $\epsilon_{m}\in [\sqrt{\frac{e}{T}},1]$, in $(d)$ we substitute the values of $\rho$ and $\psi$, $(e)$ follows from the inequality $\log T \leq \sqrt{T}$ and $(f)$ is valid for any $\epsilon_{m_i}\in[\sqrt{\frac{e}{T}},1]$. 
