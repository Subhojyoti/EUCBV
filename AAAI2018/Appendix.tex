...
where, in $(a)$ we substitute the values of $\rho$ and $\psi$, $(b)$ follows from the inequality $\log T \leq \sqrt{T}$ and $(c)$ is valid for any $\epsilon_{m_i}\in[\sqrt{\frac{e}{T}},1]$. 
...
Now,  with  $\Delta_i =\Delta = \sqrt{\frac{K\log K}{T}}>\sqrt{\frac{e}{T}}$ we obtain,
	\begin{align*}
	&\sum_{i\in \A :\Delta_{i} > b}\dfrac{320\sigma_i^2\log{(\frac{T\Delta_{i}^{2}}{K})}}{\Delta_{i}} \leq  \dfrac{320\sigma_i^2 K\sqrt{T}\log{(T\dfrac{K(\log K)}{T K})}}{\sqrt{K\log K}}\\ 
	&\leq  \dfrac{320\sigma_i^2\sqrt{KT}\log{(\log K)}}{\sqrt{\log K}}
	\overset{(a)}{\leq} 320\sigma_i^2\sqrt{KT} 
	\end{align*}		
