...
\begin{proof}
For each sub-optimal arm ${i}\in\mathcal{A}$, let $m_{i}=\min{\left\lbrace m|\sqrt{4\epsilon_{m_i}} < \frac{\Delta_{i}}{4}\right\rbrace}$. Also, let $\A^{'}=\lbrace i\in \A: \Delta_{i} > b \rbrace$ and $\A^{''}=\lbrace i\in \A: \Delta_{i} > 0 \rbrace$. Note that as all rewards are bounded in $[0,1]$, it implies that $\sigma_i^2 \leq \frac{1}{4},\forall i\in \A$. Now, as in \cite{auer2010ucb}, we bound the regret under the following two cases: 
\begin{itemize}
...
We obtain $(a)$ by substituting the value of $\psi$, $(b)$ from $0\leq\sigma_i^2 \leq\frac{1}{4}$ and $(c)$ from Lemma \ref{proofTheorem:Lemma:6}.\\
