...
%any arm $i$ cannot be pulled more than $z_i= n_{m_i}$ times or it will get eliminated. This is because in the $m_i$-th round $n_{m_{i}}=\dfrac{\log{(\psi T\epsilon_{m_{i}}^{2})}}{2\epsilon_{m_{i}}}$ and putting this in $c_{m_i}$ we get,
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%
	 & \overset{(b)}{\leq} \sqrt{2\rho\epsilon_{m_{i}} + 2.\frac{3}{2}\epsilon_{m_i}} 
	  ...
	  < \dfrac{\Delta_{i}}{4} 
	\end{align*}
In the above $(a)$ happens because $\hat{v}_i \in [0,1]$, $\rho \in (0,\frac{1}{2}]$ and $(b)$ occurs by applying the result from Lemma \ref{results:Lemma:1}. Similarly, we can also show that in the $m_i$-th round $c^* < \frac{\Delta_i}{4}$.
...
\leq \sum_{i\in \A^{'}}\bigg(\dfrac{2^{2+2\rho}.16^{\rho}}{\psi^{\rho}\Delta_{i}^{2\rho -1}}\bigg)\\   
...
\leq\Delta_{i}\bigg\lceil\dfrac{\log{(\psi T(\dfrac{\Delta_{i}}{16 \times 256})^{4})}}{2(\dfrac{\Delta_{i}}{4\sqrt{2}})^{2}}\bigg\rceil \text{, since } \sqrt{4\epsilon_{m_{i}}} < \dfrac{\Delta_{i}}{4}\\
&\leq\Delta_{i}\bigg(1+\dfrac{64\log{(\psi T(\dfrac{\Delta_{i}^{4}}{16384})}}{\Delta_{i}^{2}}\bigg)
\leq \Delta_{i}\bigg(1+\dfrac{64\log{(\psi T\Delta_{i}^{4})}}{\Delta_{i}^{2}}\bigg)
\end{align*} 
 
Summing over all arms in $\A^{'}$ the total regret is given by, 
\begin{align*}
\sum_{i\in \A^{'}}\Delta_{i}\bigg(1+\dfrac{64\log{(\psi T\Delta_{i}^{4}})}{\Delta_{i}^{2}}\bigg) = \sum_{i\in \A^{'}}\bigg(\Delta_{i} +\dfrac{64\log{(\psi T\Delta_{i}^{4}})}{\Delta_{i}}\bigg)
\end{align*}
...
\begin{align*}
 \E [R_{T}] \leq &\sum\limits_{i\in \A :\Delta_{i} > b}\bigg\lbrace\bigg(\dfrac{C_{1}(\rho)T^{1-\rho}}{\Delta_{i}^{2\rho -1}}\bigg) + \bigg(\Delta_{i}+\dfrac{64\log{(\psi  T\Delta_{i}^{4})}}{\Delta_{i}}\bigg) + \bigg(\dfrac{ C_{2}(\rho) T^{1-\rho}}{\Delta_{i}^{2\rho-1}} \bigg)\bigg \rbrace\\ 
...
After putting the parameter values $\psi=\frac{T}{K^2}$ and $\rho=\frac{1}{2}$ in the above Theorem \ref{Result:Theorem:1} result we get,
	...
	\begin{align*}
	\sum_{i\in \A :\Delta_{i} > b}\dfrac{64\log{(\psi T\Delta_{i}^{4})}}{\Delta_{i}}=\sum_{i\in \A :\Delta_{i} > b}\dfrac{128\log{(\dfrac{T\Delta_{i}^{2}}{K})}}{\Delta_{i}}
	...
	\begin{align*}
	& \sum_{i\in \A :\Delta_{i} > b}\bigg\lbrace 96 K + \dfrac{128\log{(\dfrac{T\Delta_{i}^{2}}{ K})}}{\Delta_{i}}\bigg\rbrace 
...
Putting the parameter values $\psi=\frac{T}{K^2}$ and $\rho=\frac{1}{2}$ in the result of above Theorem \ref{Result:Theorem:1} we get,
	...
	\begin{align*}
	\sum_{i\in \A :\Delta_{i} > b}\dfrac{64\log{(\psi T\Delta_{i}^{4})}}{\Delta_{i}} &= \dfrac{64K\sqrt{T}\log{(T^{2}\dfrac{K^{2}(\log K)^{2}}{T^{2} K^2})}}{\sqrt{K\log K}} \leq  \dfrac{128\sqrt{KT}\log{(\log K)}}{\sqrt{\log K}}\\
	&\leq 128\sqrt{KT} \text{ ,   as $\dfrac{\log{(\log K)}}{\sqrt{\log K}}\leq 1$}
	\end{align*}		
	%for $K\geq 3$
 	Lastly we can bound the error terms as, 
	\begin{align*}
