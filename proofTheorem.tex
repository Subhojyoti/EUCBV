...
\end{align*}
But, for $T\geq K^{2.67}$, this is clearly not possible. Hence, $\dfrac{m \log(2)}{\log(\psi T) - 2m\log( 2)} \leq 1$.
\end{proof}
...
&\leq\Delta_{i}\bigg(1+\dfrac{9\log{(\psi T(\dfrac{\Delta_{i}^{4}}{162})}}{\Delta_{i}^{2}}\bigg)
...
Putting the parameter values $\psi=\frac{T}{K^2}$ and $\rho=\frac{1}{2}$ in the result of above Theorem \ref{Result:Theorem:1} we get,
	...
	\begin{align*}
	\sum_{i\in \A :\Delta_{i} > b}\dfrac{41\log{(\psi T\Delta_{i}^{4})}}{\Delta_{i}} &= \dfrac{9K\sqrt{T}\log{(T^{2}\dfrac{K^{2}(\log K)^{2}}{T^{2} K^2})}}{\sqrt{K\log K}} \leq  \dfrac{18\sqrt{KT}\log{(\log K)}}{\sqrt{\log K}}\\
