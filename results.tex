...
\emph{Discussion:} In the non-stochastic scenario, Auer et al.\ in \cite{auer2002nonstochastic} showed that the bound on the cumulative regret for EXP-3 is $O\left(\sqrt{KT\log K}\right)$. However, in the stochastic case, UCB1 proposed in \cite{auer2002finite} incurred a regret of order of  $O\left(\sqrt{KT\log T}\right)$ which is clearly improvable. From the above result we see that in the gap-independent bound of EUCBV the most significant term is $O\left(\sqrt{KT}\right)$ which  matches the upper bound of MOSS and OCUCB, and is better than UCB-Improved, UCB1 and UCBV.
