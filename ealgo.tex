...
\For{$t=K+1,..,T$}	
\State Pull arm $i\in \argmax_{j\in B_{m}}\bigg\lbrace \hat{r}_{j} + \sqrt{\frac{\rho(\hat{v}_{j}+1)\log{(\psi T\epsilon_{m})}}{4 z_{j}}} \bigg\rbrace$, where $z_j$ is the number of times arm $j$ has been pulled
\State $t:=t+1$
\ArmElim
...
%%%%%%%%%%%%%%%%%%%%%%%
& \hat{r}_{i} + \sqrt{\frac{\rho(\hat{v}_{i}+1)\log{(\psi T\epsilon_{m})}}{4 z_{i}}}  < \max_{{j}\in B_{m}}\bigg\lbrace\hat{r}_{j} -\sqrt{\frac{\rho(\hat{v}_{j}+1)\log{(\psi T\epsilon_{m})}}{4 z_{j}}} \bigg\rbrace
\end{align*}
%\State $|B_{m}|:=|B_{m}|-1$
\EndArmElim
